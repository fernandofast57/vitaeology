...
DECLARE
  ...
  v_abbonamento VARCHAR(20);
BEGIN
  -- Conta clienti attivi (subscription attive portate dall'affiliato)
  SELECT COUNT(DISTINCT sat.user_id)
  INTO v_clienti_attivi
  FROM subscription_affiliate_tracking sat
  JOIN subscriptions s ON sat.subscription_id = s.id
  ...
  SET 
    ...
    bonus_performance = get_bonus_performance(v_clienti_attivi),
    bonus_milestone_mensile = get_bonus_milestone_mensile(v_clienti_attivi),
    commissione_percentuale = get_commissione_base_da_abbonamento(COALESCE(v_abbonamento, 'leader')) + get_bonus_performance(v_clienti_attivi),
...
CREATE OR REPLACE FUNCTION trigger_update_affiliate_on_subscription_change()
RETURNS TRIGGER AS $$
DECLARE
  v_affiliate_id UUID;
BEGIN
  -- Trova affiliato collegato a questo utente
...
DROP TRIGGER IF EXISTS trg_update_affiliate_on_subscription ON subscriptions;

CREATE TRIGGER trg_update_affiliate_on_subscription
AFTER INSERT OR UPDATE OF status, plan_type ON subscriptions
FOR EACH ROW
EXECUTE FUNCTION trigger_update_affiliate_on_subscription_change();
