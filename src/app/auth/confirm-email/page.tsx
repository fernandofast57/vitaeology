...
function ConfirmEmailForm() {
  ...
  useEffect(() => {
    if (!emailParam) {
      const stored = sessionStorage.getItem('confirmEmail')
      if (stored) {
        setEmail(stored)
      } else {
        // Se non c'è email, torna al login
  ...
  const handleVerify = async (fullCode?: string) => {
    ...
    if (!email) {
      setError('Email non trovata. Torna al login.')
      return
    }
    ...
    try {
      const res = await fetch('/api/auth/verify-confirmation-otp', {
        method: 'POST',
      ...
      // Successo!
      setSuccess(true)
      sessionStorage.removeItem('confirmEmail')
        ...
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          <div className="bg-white py-8 px-4 shadow-sm rounded-xl sm:px-10 text-center">
            ...
            <p className="text-gray-600 mb-6">
              Ora puoi accedere con le tue credenziali. Verrai reindirizzato al login...
            </p>
